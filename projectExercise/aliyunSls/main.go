...
func main() {
	...
	//var Client Client
	//res, _ := yaml.Marshal(&Client)
	//ioutil.WriteFile(confFile, res, 0644)
	client, err := UnmarshalConfig()
	if err != nil {
		logger.Info("unmarshal config file err :", zap.Error(err))
	}
	client.consumerGroupOperation()
	client.PullDataByConsumer()
	return
}
